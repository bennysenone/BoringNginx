...
# Download additional Nginx modules (for each of these we need to include an --add-module line in the Nginx configuration step)
    && git clone --depth 1 https://github.com/openresty/headers-more-nginx-module.git /usr/src/ngx_headers_more \
    && git clone --depth 1 https://github.com/yaoweibin/ngx_http_substitutions_filter_module.git /usr/src/ngx_subs_filter \
    \
# Download and prepare Nginx
    && CONFIG="\
		...
		--add-module=/usr/src/ngx_subs_filter \
	" \
	&& addgroup -S nginx \
